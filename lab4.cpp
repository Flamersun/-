...
{
	setlocale(LC_ALL, "");
	cout << "Сгенерированые ключи:" << endl << endl;
	...
	cout << '(' << setw(5) << e << ',' << setw(5) << n << ')' << " - Открытый ключ" << endl;
	cout << '(' << setw(5) << d << ',' << setw(5) << n << ')' << " - Секретный ключ" << endl << endl;
	const int MAX = 30;
	char* Text = new char[MAX];
	cout << "Введите текст для шифровки и нажмите Enter" << endl;
	cin.get(Text, MAX);
	unsigned int* CryptoText = new unsigned int[MAX];
	unsigned int* Tdecrypt = new unsigned int[MAX];
	int b = 301;
	int c;
	cout << endl << setw(5) << "Текст" << setw(6) << "ASCII"
		<< setw(20) << "Криптотекст" << setw(14)
		<< "ASCII расшофровка" << setw(14) << "Расшифровка текста" << endl
		<< "------------------------------------------------------------" << endl;
	for (int j = 0; j < MAX; j++)
	{
		c = 1;
